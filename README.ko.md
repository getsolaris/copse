...
- **업데이트 확인** — 명시적 확인 후 copse 업데이트 확인/설치 (`copse update`)
...
| `copse update` | copse 업데이트 확인 및 설치 |
...
새 copse 릴리스를 확인하고 감지된 설치 방식에 맞게 업데이트합니다.

```bash
copse update              # 설치 전 확인 프롬프트
copse update --check      # 확인만 수행
copse update --yes        # 프롬프트 없이 설치
copse update --json       # 머신 판독 가능 상태 출력
copse update --ignore     # 현재 최신 버전 무시
```

설치 경로:

- Homebrew 설치는 `brew upgrade getsolaris/tap/copse`를 실행합니다.
- Bun global 설치는 `bun install -g @getsolaris/copse@<version>`을 실행합니다.
- npm global 설치는 `npm install -g @getsolaris/copse@<version>`을 실행합니다.
- standalone 설치는 릴리스 asset에 sha256 digest가 있을 때만 현재 바이너리를 교체합니다.
- source/dev checkout은 덮어쓰지 않습니다. git으로 업데이트하거나 copse를 다시 설치하세요.

인터랙티브 CLI 명령과 TUI는 설정된 확인 간격이 지난 뒤 같은 업데이트 프롬프트를 보여줄 수 있습니다. JSON/porcelain 출력, non-TTY 실행, `copse update`, `copse init`, help, version 출력에서는 시작 시 프롬프트를 건너뜁니다.

copse는 silent update를 수행하지 않습니다. 백그라운드 daemon을 실행하지 않고 telemetry를 보내지 않으며, 모든 설치는 `copse update --yes` 또는 인터랙티브 프롬프트의 명시적 확인으로만 시작됩니다.
...
- 심볼릭 링크는 따라가지 않습니다.
- 해석된 config는 프로세스 안에서 config 파일 stat 기준으로 캐시됩니다. 쓰기와 `copse init`은 캐시를 무효화합니다. 이미 실행 중인 프로세스에서 새 workspace 레포를 반영하려면 프로세스를 재시작하거나 config 파일을 touch 하세요.
...
명시적 `copse update` 명령과 opt-in 인터랙티브 CLI 시작 프롬프트/TUI 업데이트 모달에 사용하는 업데이트 확인 설정입니다.
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `enabled` | `boolean` | `false` | opt-in 인터랙티브 시작 시/TUI 업데이트 확인 활성화. 비활성화해도 명시적 `copse update`는 동작합니다. |
| `checkIntervalHours` | `number` | `24` | 성공한 시작 시/TUI 확인 사이의 최소 시간. 양의 정수여야 합니다. |
| `ignoredVersion` | `string` | — | `copse update --ignore` 또는 TUI ignore 동작 후 숨길 최신 버전. |

시작 시 업데이트 확인은 non-TTY 실행, 머신 판독 가능 출력, `copse update`, `copse init`, help, version 출력에서는 건너뜁니다. 설치 프롬프트는 `copse update --yes`를 쓰지 않는 한 항상 명시적 확인이 필요합니다.
