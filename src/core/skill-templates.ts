...
const commandOverview: CommandOverview[] = [
  ...
  { command: "update", aliases: "-", description: "Check for and install copse updates with explicit prompts." },
...
const referenceSpecs: ReferenceSpec[] = [
  ...
  {
    command: "update",
    summary: "Check for and install copse updates.",
    synopsis: "copse update",
    options: [
      { flag: "--check", type: "boolean", alias: "-", description: "Check for updates without installing" },
      { flag: "--yes", type: "boolean", alias: "-y", description: "Install without prompting" },
      { flag: "--json", type: "boolean", alias: "-j", description: "Output as JSON" },
      { flag: "--ignore", type: "boolean", alias: "-", description: "Ignore the current latest version" },
    ],
    examples: [
      ["Check without installing", "copse update --check"],
      ["Install after confirmation", "copse update"],
      ["Install non-interactively", "copse update --yes"],
    ],
    notes: [
      "Detects Homebrew, Bun global, npm global, standalone, and source/dev install methods.",
      "Source/dev checkouts are not overwritten; update them with git or reinstall copse.",
      "Standalone updates require a release asset with a sha256 digest.",
      "Interactive launch-time and TUI prompts use the same update checker but skip scripts and machine-readable output.",
      "copse never performs a silent update, never runs a daemon, and sends no telemetry.",
    ],
    configKeys: [
      "`updates.enabled` — Enable opt-in interactive launch-time and TUI update prompts",
      "`updates.checkIntervalHours` — Minimum hours between successful launch-time/TUI checks",
      "`updates.ignoredVersion` — Latest version to suppress after ignoring an update",
    ],
  },
  {
    command: "open",
...
export function generateSkillContent(): string {
  ...
  return [
    ...
    "- Update prompts are controlled by `updates.enabled`, `updates.checkIntervalHours`, and `updates.ignoredVersion`",
    ...
    "- copse never performs a silent update, runs a daemon, or sends telemetry",
    "",
    "## Command Reference",
    "",
...
function generateConfigSchemaContent(): string {
  return [
    ...
    "| Key | Type | Default | Description |",
    "|-----|------|---------|-------------|",
    "| `enabled` | boolean | `false` | Enable opt-in interactive launch-time and TUI update prompts. Explicit `copse update` still works when disabled. |",
