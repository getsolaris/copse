...
function generateConfigSchemaContent(): string {
  return [
    ...
    "|-----|------|---------|-------------|",
    "| `terminalCommand` | string[] | platform default | Argv template for the TUI `t` shortcut. `{path}` is replaced with the selected worktree path; if omitted, the path is appended. |",
